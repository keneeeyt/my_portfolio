...
const config = {
  ...
  plugins: [
    ...
    function ({ matchUtilities, theme }: any) { // eslint-disable-line
      matchUtilities(
        {
          "bg-grid": (value: any) => ({ // eslint-disable-line
            backgroundImage: `url("${svgToDataUri(
              `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="100" height="100" fill="none" stroke="${value}"><path d="M0 .5H31.5V32"/></svg>`
            )}")`,
          }),
          "bg-grid-small": (value: any) => ({ // eslint-disable-line
            backgroundImage: `url("${svgToDataUri(
              `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="8" height="8" fill="none" stroke="${value}"><path d="M0 .5H31.5V32"/></svg>`
            )}")`,
          }),
          "bg-dot": (value: any) => ({ // eslint-disable-line
            backgroundImage: `url("${svgToDataUri(
...
function addVariablesForColors({ addBase, theme }: any) { // eslint-disable-line
  let allColors = flattenColorPalette(theme("colors")); // eslint-disable-line
  let newVars = Object.fromEntries( // eslint-disable-line
